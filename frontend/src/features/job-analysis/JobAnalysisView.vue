<template>
  <div id="job-analysis-content" class="u-stack">
    <div class="u-row u-row-start">
      ...
      <div class="u-inline header-actions">
        <el-button type="primary" plain :loading="exportingPdf" @click="handleExportPDF">
      ...
      <el-card class="filter-card" shadow="never">
        <el-form :model="filters" label-position="top">
          ...
          <div class="filter-actions">
            <el-button type="primary" class="search-btn" :loading="loading" @click="handleSearch">立即查询</el-button>
      ...
      <el-card class="table-card" shadow="never">
        ...
        >
          ...
          <el-table-column prop="city" label="城市" width="100">
            <template #default="{ row }">
              <div class="city-tag">
                <el-icon><Location /></el-icon> {{ row.city }}
              </div>
            </template>
          </el-table-column>
...
const filteredJobs = computed(() => {
  ...
  return jobs.value.filter((job) => job.jobName?.toLowerCase().includes(search) || job.companyName?.toLowerCase().includes(search))
})
...
const loadJobs = async () => {
  ...
  try {
    const data =
      currentSource.value === '51job'
        ? await jobsApi.getJobPage51Data(currentPage.value, pageSize.value, getApiFilters())
        : await jobsApi.getJobPageData(currentPage.value, pageSize.value, getApiFilters())
    jobs.value = data?.records || []
    total.value = data?.total || 0
  } catch (e) {
    const msg = e?.message || '加载岗位列表失败'
